...
public interface Demuna_FichaRepository extends JpaRepository<Demuna_FichaEntity, Integer> {
    ...
    @Query("select f from Demuna_FichaEntity f where (trim(f.usu_registro)=trim(:usu_registro)) and  (f.codi_depe_tde=:codi_depe_tde) order by  f.departamento_demuna asc,f.provincia_demuna asc,f.distrito_demuna asc,  f.estado desc")
    ...
    @Query(value = "select nvl(max(id_ficha),0) + 1 from siga.demuna_ficha", nativeQuery = true)
    ...
    @Query("select upper(trim(w.des_departament)) from Demuna_EstablecimientosEntity w group by w.des_departament order by w.des_departament asc")
    public List<String> listarRegiones();

    @Query("select f  from Demuna_FichaEntity f where ('00'=trim(:codi_depa_dpt) or trim(f.departamento_demuna)=trim(:codi_depa_dpt))  and  ('00'=:estado or f.estado =:estado) order by f.departamento_demuna asc,f.provincia_demuna asc,f.distrito_demuna asc,  f.estado desc")
    ...
    @Query(value = "select upper(trim(w.des_departament)) as region,\n"
            ...
            + "from siga.demuna_padron w group by upper(trim(w.des_departament)), w.coddptox order by w.coddptox asc", nativeQuery = true)
    ...
    @Transactional
    @Modifying
    @Query(value = "update siga.demuna_ficha f set f.estado_s1 = '1', f.usu_valida=:usu_valida, f.fch_valida=sysdate  where f.id_ficha=:id_ficha", nativeQuery = true)
    ...
    @Query(value = "SELECT SUM(N.ESTADO) CANT_ESTADO\n"
            ...
            + "FROM SIGA.DEMUNA_FICHA_S11 F  WHERE F.ID_FICHA=:id_ficha) N", nativeQuery = true)
    int findSumOfStatesByFichaId(@Param("id_ficha") Integer id_ficha);

    @Transactional
    @Modifying
    @Query(value = "update siga.demuna_ficha f set f.flag_validar=:flag_validar, f.usu_valida=:usu_validar, f.fch_valida=sysdate where f.id_ficha=:id_ficha", nativeQuery = true)
